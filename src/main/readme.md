...
toString メソッドはテストコードなしで実装した理由としては以下の3点
TDDの観点では掟破りではあるが、今回は「画面の出力をすぐに見る」、  
「toStringメソッドはデバッグ出力のために利用されるので、バグが入るリスクが少ない」、  
「テストがレッドばーの状態だったので、その状態でテストを作成するのは避けたい」 
  
サブクラスにコンストラクタしか存在しない状況で完了  

## 11章

ついに、サブクラスが不要になったので削除しました。  
サブクラスの仕事を減らし続けて、冗長になったテストを削除できた。  

サブクラスを消すなら  
- 似たようなサブクラスがあれば実装を合わせられれないか確認する
- 合わせれたら、メソッドは親クラスに持たせて実行できるように修正する
- テストして問題なく動作するなら削除する
...
TDDを進めていると、テストコードとプロダクトコードの行数は同じくらいになる。  
つまり、これまでの2倍コードを書けるようになるか、半分のコード量で同じだけの機能を書けるようになるか  
しないとTDDのメリットを得られない  
